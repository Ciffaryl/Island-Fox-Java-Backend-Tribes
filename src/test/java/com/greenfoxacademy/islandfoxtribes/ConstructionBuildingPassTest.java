...
public class ConstructionBuildingPassTest extends TestSetup {
    ...
    @Test
    public void buildNewBuildingTest() throws Exception {
        Mockito.when(buildingService.buildBuilding("Farm", kingdom)).thenReturn(null);
    ...
    public void upgradeBuilding() throws Exception {
        Mockito.when(playerServiceImpl.validation(1L)).thenReturn(true);
        Mockito.when(buildingService.buildBuilding(1L, kingdom)).thenReturn(null);
        mockMvc.perform(MockMvcRequestBuilders.post("/kingdoms/1/buildings/1").
                        contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "Bearer " + jwt.getToken())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
